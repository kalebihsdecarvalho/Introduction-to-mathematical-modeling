...
alfa = np.linalg.solve(A, valores)

def interpolador_rbf(ponto, pontos, alfa):
    distancias = np.linalg.norm(pontos - ponto, axis=1)
    return np.sum(alfa * np.exp(-distancias**2))

x, y = map(float, input().split())
ponto = np.array([x, y]) 

valor = interpolador_rbf(ponto, pontos, alfa)
